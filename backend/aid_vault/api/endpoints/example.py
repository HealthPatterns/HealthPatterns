...
def get_user(db: SessionInstance, user_id: int):
    user_in_db = crud_example.read_user(db, user_id)
    if not user_in_db:
...
def put_user(db: SessionInstance, user_id: int, user: UserSchema, response: Response):
    ...
    if user_in_db:
        if user_in_db.email != user.email and crud_example.read_user_by_email(db, email=user.email):
            raise HTTPException(status_code=400, detail="Email already registered.")
        return crud_example.update_user(db, user)
    # if user doesn't exist:
    if crud_example.read_user_by_email(db, email=user.email):
        raise HTTPException(status_code=400, detail="Email already registered.")
    response.status_code = status.HTTP_201_CREATED
    return crud_example.create_user(db, user)
...
def delete_user(db: SessionInstance, user_id: int):
    ...
    if not user_in_db:
        raise HTTPException(status_code=404, detail="Not found.")
    crud_example.delete_user(db, user_id)